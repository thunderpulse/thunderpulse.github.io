...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus('error');
      setErrorMessage('Network Error: Failed to send message. Please check connection.');
    }
  };
    ...
    <section id="contact" className="py-16 sm:py-20 md:py-28 bg-section-base relative overflow-hidden">
      ...
      <div className="section-container relative z-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="relative">
                <input 
                  ...
                  required
                  className="block w-full px-4 pt-6 pb-2 text-sm text-white bg-white/5 border border-white/10 rounded-xl appearance-none focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500 peer transition-all"
                  placeholder=" "
                />
                <label 
                  htmlFor="name" 
                  className="absolute text-sm text-white/50 duration-300 transform -translate-y-3 scale-75 top-4 z-10 origin-[0] left-4 peer-focus:text-violet-400 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-3"
                >
                  Name
                </label>
              </div>
              ...
              <div className="relative">
                <textarea 
                  ...
                  required
                  rows="4"
                  className="block w-full px-4 pt-6 pb-2 text-sm text-white bg-white/5 border border-white/10 rounded-xl appearance-none focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500 peer transition-all resize-none"
                  placeholder=" "
                />
                <label 
                  htmlFor="message" 
              ...
              {status === 'success' && (
                <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-lg text-green-400 text-sm flex items-center gap-2">
                  ...
                  Message securely logged and sent!
                </div>
              )}
              ...
              <button 
                ...
                className="w-full bg-violet-600 hover:bg-violet-700 text-white font-medium py-3.5 px-6 rounded-xl transition-all duration-300 transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2 shadow-lg shadow-violet-600/20"
              >
                {status === 'loading' ? (
                  <>
        ...
        <p className="text-center text-white/15 text-sm mt-20">
          © {new Date().getFullYear()} Pinaki Rath · Built with React & Node.js
        </p>
      </div>
    </section>
  );
